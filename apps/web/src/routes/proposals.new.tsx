...
function DraftProvider({ children }: { children: ReactNode }) {
  ...
  const valid: Record<ChapterId, boolean> = {
    ...
    case:
      problem.trim().length >= 40 &&
      solution.trim().length >= 40 &&
      outcomes.trim().length >= 40,
    ledger: costsValid && (!timelineRequired || filledMilestones.length > 0),
    voices: !!address && isConnected,
    seal: true,
  }
  const allValid =
    valid.motion && valid.case && valid.ledger && valid.voices && !busy
  ...
  const actions: DraftActions = {
    ...
    addCost: () =>
      setCosts((cs) => [...cs, { id: newId(), item: '', amount: '' }]),
    updateCost: (id, patch) =>
      setCosts((cs) => cs.map((c) => (c.id === id ? { ...c, ...patch } : c))),
    removeCost: (id) =>
      setCosts((cs) => (cs.length === 1 ? cs : cs.filter((c) => c.id !== id))),
    ...
    removeMilestone: (id) =>
      setMilestones((ms) =>
        ms.length === 1 ? ms : ms.filter((m) => m.id !== id),
      ),
  ...
  return (
    <DraftContext value={{ state, actions, meta }}>{children}</DraftContext>
  )
}
    ...
    >
      ...
      <section className="mt-10 grid gap-12 sm:mt-12 lg:grid-cols-[minmax(0,1fr)_22rem] lg:gap-16">
        <div className="min-w-0">
          ...
          <div
            key={state.chapterIdx}
            className="[animation:folio-in_440ms_cubic-bezier(.2,.7,.2,1)_both]"
          >
            <ActiveFolio />
          </div>
        </div>
      ...
      <div className="flex items-end gap-3 sm:gap-4">
        <span
          ...
          style={{
            border: ok
              ? '1.5px solid var(--ink)'
              : '1.5px solid color-mix(in oklch, currentColor 18%, transparent)',
...
function LedgerFolio() {
  ...
  return (
    <FolioShell
      num="03"
      name="The ledger"
      tagline="Every line of money accounted for, every milestone scheduled."
    >
      <section>
        <SectionHead
          tag="I"
  ...
  return (
    ...
    >
      <input
        ...
        placeholder={
          index === 0 ? 'e.g. Honorarium — workshop facilitator' : 'Line item'
        }
        disabled={busy}
      ...
      <div className="flex items-center justify-end gap-3 sm:shrink-0">
        <input
          value={line.amount}
          onChange={(e) =>
            updateCost(line.id, { amount: sanitiseAmount(e.target.value) })
          }
          inputMode="decimal"
          placeholder="0.00"
          disabled={busy}
      ...
      <section>
        ...
        {address && isConnected ? (
          ...
          <p
            ...
            style={{
              border:
                '1px dashed color-mix(in oklch, currentColor 22%, transparent)',
            }}
          >
...
function CommunityPicker() {
  ...
  const selectedSet = useMemo(
    () => new Set(coAuthors.map((a) => a.toLowerCase())),
    [coAuthors],
  )
...
function SealFolio() {
  ...
  return (
    <FolioShell
      num="05"
      name="The seal"
      tagline="A last look at the cover, then the motion is filed."
    >
      ...
      <div className="hidden lg:block">
        <p className="border border-dashed border-border bg-secondary/30 p-5 font-serif text-[14px] italic leading-relaxed text-muted-foreground">
          ...
          , three things happen: you sign your intent, the brief pins to IPFS,
          and the content identifier is recorded on-chain. Submission is
          sponsored — you pay nothing.
        </p>
      </div>
      ...
      <p className="mt-6 font-serif text-[13px] leading-relaxed text-muted-foreground">
        On the record, the full brief is pinned to IPFS; only its content
        identifier is stored on-chain. On gas, submission is sponsored — members
        vote in confidence.
      </p>
    </FolioShell>
  )
  ...
  return (
    ...
    >
      ...
      <div className="relative mt-5 border-t border-border pt-4">
        ...
        <DossierRow
          label="Milestones"
          value={filledMilestonesCount ? String(filledMilestonesCount) : '—'}
        />
  ...
  const nextLabel = isLast
    ? filed
      ...
      : busy
        ...
        : 'File motion'
    : 'Next'
  const microStatus = error
    ? `Error · ${error}`
    : busy
      ? PASSAGE.find((s) => s.key === phase)?.label
      : null
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center gap-2 px-3 py-3 sm:gap-3 sm:px-6">
        ...
        >
          ...
          <span
            aria-hidden
            className="shrink-0 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground"
          >
            ⌃
          </span>
        </button>
