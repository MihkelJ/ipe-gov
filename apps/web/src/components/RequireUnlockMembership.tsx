...
import { UNLOCK_CHECKOUT_URL, useUnlockMembership } from "#/hooks/useUnlockMembership";
...
export default function RequireUnlockMembership({ children }: { children: React.ReactNode }) {
...
function Gate({
  ...
  const stepIndex =
    stage === "identity" ? 0 : stage === "wallet" ? 1 : stage === "checking" || stage === "key" ? 2 : -1;
  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-4 pb-24 pt-8 sm:px-6 sm:pt-12">
      ...
      <header>
        ...
        <p className="mt-4 max-w-prose text-sm text-muted-foreground sm:text-[15px]">{copy.body}</p>
      </header>
      ...
      <div className="mt-8 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
        <ActionButton stage={stage} onLogin={onLogin} onConnect={onConnect} cta={copy.cta} />
        {stage === "key" ? (
...
function StepRow({ index, step, status }: { index: number; step: (typeof STEPS)[number]; status: StepStatus }) {
  return (
    <li className={cn("flex items-center gap-3 py-3 sm:gap-5 sm:py-4", status === "pending" && "opacity-55")}>
      ...
      <div className="flex min-w-0 flex-1 items-baseline justify-between gap-3">
        <div className="min-w-0">
          <div className="truncate text-sm font-medium sm:text-base">{step.label}</div>
          <div className="mt-0.5 truncate text-[11px] text-muted-foreground sm:text-xs">{step.detail}</div>
        </div>
        <span
          className={cn(
...
function ActionButton({
  ...
  return (
    <Button size="lg" variant="secondary" disabled className="w-full sm:w-auto">
      <span aria-hidden className="mr-2 inline-block size-1.5 animate-pulse rounded-full bg-foreground" />
      {cta}
    </Button>
  );
}
