...
export function AddressIdentity({ address, size = "sm", className }: AddressIdentityProps) {
  ...
  const fallback = name ? name.slice(0, 2).toUpperCase() : address.slice(-4, -2).toUpperCase();

  return (
    <div className={`flex items-center gap-3${className ? ` ${className}` : ""}`}>
      <Avatar size={size}>
        {avatarUrl ? <AvatarImage src={avatarUrl} alt={label} /> : null}
        <AvatarFallback className="font-mono text-[10px] uppercase tracking-wider">{fallback}</AvatarFallback>
      </Avatar>
