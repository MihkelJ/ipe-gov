...
// The paymaster-proxy routes by chainId in the path. Sponsored writes target
// Sepolia; Base/Mainnet writes are not gas-sponsored.
const PAYMASTER_URL = PAYMASTER_BASE ? `${PAYMASTER_BASE}/${sepolia.id}` : undefined;

/**
 * Submit a contract write, sponsored by Pimlico — your members don't pay gas.
 *
 *   - If the wallet advertises EIP-5792 `paymasterService` support (Coinbase
 *     Smart Wallet, Safe, Privy embedded, etc.), the bundle is handed straight
 *     to the wallet via `wallet_sendCalls`; the wallet coordinates sponsorship
 *     with the paymaster.
 ...
 *
 * The paymaster-proxy worker still gates sponsorship on Unlock-key membership
 * server-side, so non-members won't get sponsored down either path.
 *
 * Usage:
 *
 *   const write = useSponsoredWrite();
    ...
    mutationFn: async (params: WriteParams | WriteParams[]): Promise<Hex> => {
      ...
      if (paymasterSupported) {
        ...
        const result = await waitForCallsStatus(client, {
          ...
          // jitter even when the UserOp does land.
          timeout: 180_000,
        });
      ...
      // implements authorization signing (Privy's hook routes to the embedded
      // wallet directly or to the external wallet's wallet_signAuthorization).
