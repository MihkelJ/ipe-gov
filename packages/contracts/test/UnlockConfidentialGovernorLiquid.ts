...
describe("UnlockConfidentialGovernorLiquid", function () {
  ...
  it("reverts deployment if the delegation contract's lock does not match", async function () {
    ...
    await expect(govFactory.deploy(otherLockAddress, delegationAddress, VOTING_PERIOD)).to.be.revertedWithCustomError(
      govFactory,
      "LockMismatch",
    );
  });
  ...
  it("scopes delegations per proposal", async function () {
    await (await governor.connect(alice).propose("p1")).wait();
    await (await governor.connect(alice).propose("p2")).wait();
    await (await delegation.connect(bob).delegate(1, alice.address)).wait();
  ...
  it("rejects propose from a non-member", async function () {
    ...
    await expect(governor.connect(outsider).propose("p1")).to.be.revertedWithCustomError(governor, "NotMember");
  });
